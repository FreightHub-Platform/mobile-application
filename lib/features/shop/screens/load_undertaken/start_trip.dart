...
class _StartTripScreenState extends State<StartTripScreen> {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['data'] != null && responseData['data'].isNotEmpty) {
          ...
          setState(() {
            ...
            _routeName = 'Start trip to pickup from $consigner';
  ...
  Future<void> _updateRouteStatusAndStartTrip() async {
    try {
      ...
      final response = await updateTripStarted(driverId: driverId, routeId: routeId);
      ...
      if (response.statusCode == 200) {
        ...
        Get.off(() => const LoadUndertakenScreen());
      } else {
        // Handle error
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Accepted Route'),
      ),
